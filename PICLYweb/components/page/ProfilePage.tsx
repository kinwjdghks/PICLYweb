...
const DeleteModal = ({deleteAccount,closeModal}:{deleteAccount:()=>void,closeModal:()=>void}):React.ReactNode =>{
    ...
    return <div className="w-4/5 lg:w-1/3 lg:h-1/3 bg-picly_default rounded-lg absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
        ...
        <div className="p-8 mt-auto">
            ...
            <button className="px-4 py-[0.6rem] bg-picly_darker rounded-lg mx-2" onClick={closeModal}>아니오</button>
        </div>
    </div>
}
...
const ProfilePage = ({albumCount,close}:{albumCount:number|undefined,close:(page:page)=>void}):React.ReactNode =>{
    ...
    const loginState = sessionStorage.getItem('piclyweb_loginState');
    ...
    const inputClassName = 'w-2/3 lg:h-12 h-10 border-solid border-[1px] p-2 px-4 mb-2 border-picly_lighter box-border rounded-md text-black';
    ...
    return <PageFrame className={`lg:w-[calc(100%-16rem)] lg:right-0 w-screen min-h-max absolute bg-picly_default flex flex-col ${nanumgothic.className}`}>
