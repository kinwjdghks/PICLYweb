...
const MobileMenuTab = ({
  ...
  if(!clientSide) return <div className="w-screen h-screen bg-picly_default"></div>

  const root = document.getElementById('modalroot');
  const portal = ReactDOM.createPortal(
    <>
    {mobileMenuOpen && <BackDrop onClick={()=>setMobileMenuOpen(false)}/>}
    <div className={`fixed w-1/2 h-[100svh] right-0 top-0 bg-picly_default flex justify-center ${!mobileMenuOpen && 'translate-x-full'} transition-all duration-300`}>
